...
/**
 * Net questions a 99th-percentile paper took in each section — that is, marks
 * divided by three. Career Launcher publishes CAT 2025's percentile table with
 * this column; it is a far more useful yardstick than "attempt everything",
 * because DILR at 99 is thirteen questions right, not twenty-two attempted.
 */
export const NET_QUESTIONS_AT_99: Record<Section, number> = { VARC: 15, DILR: 13, QA: 12 };
...
export function attemptDiagnosis(
  ...
  const target = NET_QUESTIONS_AT_99[section];
  ...
  if (precise && enough) {
    return {
      verdict: "At the bar",
      fix: `${pc}% accuracy and ${netQ.toFixed(1)} net questions — a 99th-percentile ${section} paper. Hold it for three more mocks before changing anything.`,
    };
  }
  if (precise && !enough) {
    return {
      verdict: "Too cautious",
      fix: `${pc}% of what you touch is right, so accuracy is not the problem — you are ${short} net questions short of the ${target} a 99 takes. Attempt two more and watch whether it holds.`,
    };
  }
  if (!precise && enough) {
    return {
      verdict: "Paying for volume",
      fix: `You clear ${target} net questions, but at ${pc}% accuracy. Every wrong answer costs a mark and the minutes that bought it — the same score is available from fewer attempts.`,
    };
  }
  if (acc < 0.65) {
    return {
  ...
  return {
    ...
    fix: `${pc}% accuracy, ${short} net questions off ${target}. Accuracy first: past 80% is worth more than two extra attempts. ${SECTION_NOTE[section]}`,
  };
}
